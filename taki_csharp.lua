...
--
	...
	cs.elements.makefile = function(prj)
		return {
			...
			ninja.phonyRules,
			ninja.csConfigs,
			ninja.csProjectConfig,
			ninja.csSources,
			ninja.csEmbedFiles,
			ninja.csCopyFiles,
			ninja.csResponseFile,
			ninja.shellType,
			ninja.csAllRules,
			ninja.csTargetRules,
			ninja.targetDirRules,
			ninja.csResponseRules,
			ninja.objDirRules,
			ninja.csCleanRules,
			ninja.preBuildRules,
			ninja.csFileRules,
...
--
	...
	function ninja.csConfigs(prj, toolset)
...
---------------------------------------------------------------------------
	...
	function ninja.csAllRules(prj, toolset)
		_p('all: prebuild $(EMBEDFILES) $(COPYFILES) $(TARGET)')
		_p('')
	end


	function ninja.csCleanRules(prj, toolset)
		--[[
		-- porting from 4.x
		_p('clean:')
	...
	function ninja.csCopyFiles(prj, toolset)
		--[[
	...
	function ninja.csResponseFile(prj, toolset)
		_x('RESPONSE += ' .. ninja.cs.getresponsefilename(prj))
	end


	function ninja.csResponseRules(prj)
		local toolset = p.tools.dotnet
		local ext = ninja.getmakefilename(prj, true)
		local makefile = path.getname(p.filename(prj, ext))
		local response = ninja.cs.getresponsefilename(prj)
	...
	function ninja.csEmbedFiles(prj, toolset)
	...
	function ninja.csFileRules(prj, toolset)
		--[[
		-- porting from 4.x
	...
	function ninja.csFlags(cfg, toolset)
		_p('  FLAGS =%s', ninja.list(toolset.getflags(cfg)))
	end


	function ninja.csLinkCmd(cfg, toolset)
		local deps = p.esc(config.getlinks(cfg, "dependencies", "fullpath"))
		_p('  DEPENDS =%s', ninja.list(deps))
		_p('  REFERENCES = %s', table.implode(deps, "/r:", "", " "))
	end


	function ninja.csProjectConfig(prj, toolset)
	...
	function ninja.csSources(prj, toolset)
		local cfg = project.getfirstconfig(prj)
	...
	function ninja.csTargetRules(prj, toolset)
	...
	function ninja.csTools(cfg, toolset)
